...
function PostCardMeta({ isFeaturedBig, views, readTime, rating }) {
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2 text-white/80">
        <StarRating rating={rating} />
        {isFeaturedBig && <span className="text-sm font-light">({views} Views)</span>}
      </div>
      {isFeaturedBig && readTime && (
        <div className="text-sm font-medium text-white/90">{readTime} min read</div>
      )}
    </div>
  );
}

function BlogPostCard({ post, titleClassName, cardHeightClass, iconSize, isFeaturedBig, className, onSelect }) {
  const {
    title: postTitle,
    category,
    imageUrl,
    ...
    >
      ...
      <article className="relative z-0 flex items-end">
        <div className="flex flex-1 flex-col gap-3">
          <h1 className={cn("font-semibold font-serif text-white/90", titleClassName)}>
            {postTitle}
          </h1>
...
function FeaturedPostsGrid({ featuredPosts, onSelect }) {
  return (
...
export const BlogPosts = ({
  ...
  return (
    <section className={cn(
      ...
      <FeaturedPostsGrid featuredPosts={featuredPosts} onSelect={handlePostClick} />
